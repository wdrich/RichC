...
#pragma mark - POST_GET_PUT_DELETE
...
{
    [self request:@"DELETE" URL:urlString parameters:parameters completion:completion];
    return self.currentTask;
}

#pragma mark - request Method
- (void)request:(NSString *)method URL:(NSString *)urlString parameters:(id)parameters completion:(void (^)(WDBaseResponse *response))completion
{
    if (self.isLocal) {
    ...
    if (self.reachabilityManager.networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
        WDBaseResponse *response = [WDBaseResponse new];
    ...
    if ([method isEqualToString:@"GET"]) {
        ...
        self.currentTask = [self GET:urlString parameters:parameters progress:nil success:success failure:failure];
        
    }
    
    if ([method isEqualToString:@"POST"]) {
        self.currentTask = [self POST:urlString parameters:parameters progress:nil success:success failure:failure];
    }
    
    if ([method isEqualToString:@"PUT"]) {
        self.currentTask = [self PUT:urlString parameters:parameters success:success failure:failure];
    }
    
    if ([method isEqualToString:@"DELETE"]) {
        
        self.currentTask = [self DELETE:urlString parameters:parameters success:success failure:failure];
    }
...
#pragma mark - 加载本地数据
static NSString *jsonFileDirectory = @"WDLocalJsons";
- (void)requestLocal:(NSString *)urlString completion:(void (^)(WDBaseResponse *response))completion
{
...
#pragma mark - 处理数据
- (void)wrapperTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error completion:(void (^)(WDBaseResponse *response))completion
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        
        WDBaseResponse *response = [self convertTask:task responseObject:responseObject error:error];
...
#pragma mark - 包装返回的数据
- (WDBaseResponse *)convertTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error
{
    
    WDBaseResponse *response = [WDBaseResponse new];
    
    if (!RCHIsEmpty(responseObject)) {
    ...
    if ([task.response isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        response.urlString = [HTTPURLResponse URL].absoluteString;
    }
...
#pragma mark - 打印返回日志
- (void)LogResponse:(NSString *)urlString response:(WDBaseResponse *)response
{
//    NSLog(@"\n[%@]---%@\n", urlString, response);
}
...
- (void)upload:(NSString *)urlString parameters:(id)parameters formDataBlock:(NSDictionary<NSData *, RCHDataName *> *(^)(id<AFMultipartFormData> formData, NSMutableDictionary<NSData *, RCHDataName *> *needFillDataDict))formDataBlock progress:(void (^)(NSProgress *progress))progress completion:(void (^)(WDBaseResponse *response))completion
{
    ...
    [self POST:urlString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if (datas) {
            [datas enumerateKeysAndObjectsUsingBlock:^(NSData * _Nonnull data, RCHDataName * _Nonnull name, BOOL * _Nonnull stop) {
...
- (void)configSettings
{
    ...
    //自定义处理数据
    self.responseFormat = ^WDBaseResponse *(WDBaseResponse *response) {
        return response;
    };
}
...
+ (instancetype)manager {
    WDRequestManager *manager = [super manager];
    [manager configSettings];
    return manager;
}

static WDRequestManager *_instance = nil;
