...
- (NSString *)confusedUnit:(NSString *)unit
{
   return [self confusedUnit:unit confusedLength:4];
}

- (NSString *)confusedUnit:(NSString *)unit confusedLength:(NSInteger)len
{
    NSMutableString *unitStr = [NSMutableString string];
    for (NSInteger i = 0; i < len; i ++) {
        [unitStr appendString:unit];
    }
    NSMutableString *result = [NSMutableString stringWithString:self];
    if ([self length] > len + 2) {
        [result replaceCharactersInRange:(NSRange){([result length] - len) / 2, len} withString:unitStr];
    } else {
        if (self > 0) {
            if (len < [self length]) {
                [result replaceCharactersInRange:(NSRange){([result length] - len), len} withString:unitStr];
            } else {
                [result replaceCharactersInRange:(NSRange){([result length] - len) / 2, len} withString:unitStr];
            }
        } else {
...
- (NSString *)confusedOnlyFirstLetterUnit:(NSString *)unit confusedLength:(NSInteger)len
{
    ...
    if ([self length] == 0) {
        [result appendString:unitStr];
        return result;
    } else {
        NSString *temp = [self substringWithRange:(NSRange){0, 2}];
...
- (NSString*) sha1
{
    ...
    CC_SHA1(self.UTF8String, [self UTF8Length], digest);
...
- (CGSize)sizeWithFont:(UIFont *)font constrainedToSize:(CGSize)constraineSize lineBreakMode:(NSLineBreakMode)lineBreakMode
{
    CGSize size = [self boundingRectWithSize:constraineSize
...
- (CGSize)sizeWithAttributes:(NSDictionary *)attributes constrainedToSize:(CGSize)constraineSize
{
...
- (CGSize)sizeWithCoreTextAttributes:(NSDictionary *)attrs constrainedToSize:(CGSize)size
{
...
//{
//
//    [self drawInRect:rect withFont:font color:kFontGrayColor];
//    
//}

- (void)drawInRect:(CGRect)rect withFont:(UIFont *)font color:(UIColor *)color
{
    
    [self drawInRect:rect withFont:font color:color lineBreakMode:NSLineBreakByWordWrapping];
}

- (void)drawInRect:(CGRect)rect withFont:(UIFont *)font color:(UIColor *)color lineBreakMode:(NSLineBreakMode)mode
{
    
    [self drawInRect:rect withFont:font color:color lineBreakMode:NSLineBreakByWordWrapping alignment:NSTextAlignmentLeft];
    
}

- (void)drawInRect:(CGRect)rect withFont:(UIFont *)font color:(UIColor *)color lineBreakMode:(NSLineBreakMode)mode alignment:(NSTextAlignment)alignment
{
