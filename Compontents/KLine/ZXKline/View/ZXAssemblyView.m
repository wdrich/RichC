...
- (void)initConstrainsWithWidth:(CGFloat)width height:(CGFloat)height
{
    ...
    [self.candleDetailView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (PriceCoordinateIsInRight) {
            ...
            make.left.mas_equalTo(5.0f); //fix
...
- (void)drawBorder
{
    ...
    self.topBorder.lineWidth = 0.4;
    self.topBorder.path = topBorderBeizer.CGPath;
    self.topBorder.strokeColor = GrateBorderLineColor.CGColor;
    ...
    if (!DrawJustKline) {
        ...
        self.bottomBorder.lineWidth = 0.4;
        self.bottomBorder.path = bottomBorderBeizer.CGPath;
        self.bottomBorder.strokeColor = GrateBorderLineColor.CGColor;
...
//    [self drawQuotaWithType:QuotaTypeLine dataArr:VOL_MA5DataArr maxValue:maxValue minValue:minValue quotaName:@"" subName:@"MA5" lineColor:MA1Color columnColorArr:nil columnWidthType:0];
//    [self drawQuotaWithType:QuotaTypeLine dataArr:VOL_MA10DataArr maxValue:maxValue minValue:minValue quotaName:@"" subName:@"MA10" lineColor:MA2Color columnColorArr:nil columnWidthType:0];
//    [self drawQuotaWithType:QuotaTypeLine dataArr:VOL_MA20DataArr maxValue:maxValue minValue:minValue quotaName:@"" subName:@"MA20" lineColor:MA3Color columnColorArr:nil columnWidthType:0];
}
//RSI
...
- (void)updateJumpViewWithNewKlineModel:(KlineModel *)newKlineModel
{
    ...
    //根据最新的价格计算对应的point的值
    if ((newPrice>=self.klineMainView.minAssert&&newPrice<=self.klineMainView.maxAssert) && IsDisplayJumpView){
...
- (void)hidePriceView
{
    ...
    self.quotaView.hidden = YES;
}

- (void)hideCandleDetailView:(BOOL)flag
{
    self.candleDetailView.hidden = flag;
    self.quotaDetailView.hidden = flag;
}

- (void)hideRotateBtn:(BOOL)flag
{
    self.rotateBtn.hidden = flag;
}
...
- (void)shouldToReloadCurrentPositionPriceJumpViewWithPositonY:(CGFloat)positionY price:(NSString *)price
{
    ...
    [self.horizontalView updateJumpViewWithNewPrice:price backgroundColor:GrateTimeBackGroundColor];
...
- (void)shouldToReloadQuotaDetailViewWithResultString:(NSAttributedString *)QuotaResultString shouldToReloadCandleDetailViewWithMAResultString:(NSAttributedString *)MAResultString
{
    if (!DrawJustKline) {
        if (IsDisplayQuotaTypeName) {
            ...
            [self.quotaDetailView reloadQuotaDetailViewWithQuotaAttributedString:QuotaResultString];
        }
    }
    if (IsDisplayStockOrQuotaName) {
    ...
    if (IsDisplayCandelInfoInTop) {
        ...
        if (point.x>candleChartCenterX&&point.y<candleChartCenterY) {
            ...
            [self.messageBoxView mas_updateConstraints:^(MASConstraintMaker *make) {
                ...
                make.centerY.mas_equalTo(self.horizontalView).offset(height/2 + apace);
                make.right.mas_equalTo(self.timeLineView).offset(-apace);
        ...
        else if (point.x<candleChartCenterX&&point.y<candleChartCenterY)
        {
            ...
            [self.messageBoxView mas_updateConstraints:^(MASConstraintMaker *make) {
                ...
                make.centerY.mas_equalTo(self.horizontalView).offset(height/2 + apace);
                make.right.mas_equalTo(self.timeLineView).offset(width + apace);
                make.width.mas_equalTo(width);
                make.height.mas_equalTo(height);
            }];
        }
        ...
        if (klineModel.isPlaceHolder) {
            ...
            [self.messageBoxView setNeedsDisplayWithOpen:@"0.0" close:@"0.0" high:@"0.0" low:@"0.0" volume:@"0.0"];
        }else{
            NSNumberFormatter *format = [RCHHelper getNumberFormatterFractionDigits:([klineModel.volumn doubleValue] < 10 ? 3 : 0)];
            [self.messageBoxView setNeedsDisplayWithOpen:[NSString stringWithFormat:@"%.*f",self.precision,klineModel.openPrice] close:[NSString stringWithFormat:@"%.*f",self.precision,klineModel.closePrice] high:[NSString stringWithFormat:@"%.*f",self.precision,klineModel.highestPrice] low:[NSString stringWithFormat:@"%.*f",self.precision,klineModel.lowestPrice] volume:[NSString stringWithFormat:@"%@",  [NSDecimalNumber decimalNumberWithString:[format stringFromNumber:klineModel.volumn]]]];
        }
...
- (UIView *)priceMaskView
{
    if (!_priceMaskView) {
        ...
        _priceMaskView.backgroundColor = [UIColor blueColor];
...
{
    if (!_messageBoxView) {
        _messageBoxView = [[ZXMessageBoxView alloc] init];
