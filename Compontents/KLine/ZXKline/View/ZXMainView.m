...
- (void)drawTimeLine
{
    ...
    NSInteger index = self.needDrawKlineArr.count > self.validDataCount ? self.validDataCount-1: self.needDrawKlineArr.count - 1;
    
    KlineModel *lastModel = self.needDrawKlineArr[index];
...
- (void)drawCandle
{
    ...
    if (self.maxValueModel&&self.minValueModel) {
        ...
        CGPoint maxValuePoint = CGPointMake((self.maxValueModel.x-self.needDrawStartIndex)*self.candleWidth-startOffsetY+self.candleWidth/2.0, self.maxValueModel.highestPoint+(self.subViewHeight-self.candleChartHeight) - 8.0f);
        CGPoint minValuePoint = CGPointMake((self.minValueModel.x-self.needDrawStartIndex)*self.candleWidth-startOffsetY+self.candleWidth/2.0, self.minValueModel.lowestPoint+(self.subViewHeight-self.candleChartHeight) - 2.0f);
            ...
            if ([values[index] isKindOfClass:[NSNumber class]]) {
                ...
                if (idx<colorArr.count) {
                    ...
                    if ([colorDic[keys.firstObject] isKindOfClass:[UIColor class]]) {
                        ...
                        attributedColor = lightGrayTextColor;  //默认--VOL Color--
                    }
                }else{
                    //TODO
                    attributedColor = lightGrayTextColor;
                }
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        if (!candleWidth) {
            ...
            self.candleWidth = (CandleMaxWidth-CandleMinWidth)/2.0;
            //将candle宽度保存到本地
            [[NSUserDefaults standardUserDefaults] setObject:@(self.candleWidth) forKey:kCandleWidth];
        }else
        {
