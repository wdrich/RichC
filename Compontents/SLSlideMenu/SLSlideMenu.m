...
#define menuW kMainScreenWidth
#define menuH kMainScreenHeight
#define navH kAppOriginY
#define shadowColor  [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:1.0f]
#define animationTime 0
...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id)delegate direction:(SLSlideMenuDirection)direction slideOffset:(CGFloat)slideOffset allowSwipeCloseMenu:(BOOL)isAllow aboveNav:(BOOL)isAbove identifier:(NSString *)identifier object:(id)object {
    ...
    if (self) {
        ...
        [shadowView addTarget:self action:@selector(viewTap:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)showMenuViewWithDirection:(SLSlideMenuDirection)direction slideOffset:(CGFloat)slideOffset shadowY:(CGFloat)shadowY {
    ...
    if (direction == SLSlideMenuDirectionRight) {
        ...
        [self.menuView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [weakself.x_arr addObject:@(subFrame.origin.x)];
            CGFloat x_temp = [[weakself.x_arr objectAtIndex:idx] floatValue];
            // 距离右边的距离
            CGFloat l = frame.size.width - x_temp - subFrame.size.width;
            subFrame.origin.x =  0 - l - subFrame.size.width;
            [weakself.startX_arr addObject:@(subFrame.origin.x)];
            obj.frame = subFrame;
        }];
        ...
        [self.menuView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CGRect subFrame = obj.frame;
            [weakself.y_arr addObject:@(subFrame.origin.y)];
            CGFloat y_temp = [[weakself.y_arr objectAtIndex:idx] floatValue];
            // 距离上边的距离
            CGFloat l = frame.size.width - y_temp - subFrame.size.width;
            subFrame.origin.y =  0 - l - subFrame.size.width;
            [weakself.startY_arr addObject:@(subFrame.origin.y)];
            obj.frame = subFrame;
        }];
    ...
    [UIView animateWithDuration:animationTime animations:^{
        ...
        if (direction == SLSlideMenuDirectionLeft) {
            [self.menuView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                CGRect subFrame = obj.frame;
                subFrame.origin.x = [[weakself.x_arr objectAtIndex:idx] floatValue];
                obj.frame = subFrame;
            }];
        }
        ...
        if (direction == SLSlideMenuDirectionTop) {
            [self.menuView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                CGRect subFrame = obj.frame;
                subFrame.origin.y = [[weakself.y_arr objectAtIndex:idx] floatValue];
                obj.frame = subFrame;
            }];
        }
        
        self.shadowView.backgroundColor = [shadowColor colorWithAlphaComponent:0.3f];
...
//
    ...
    [UIView animateWithDuration:animationTime animations:^{
        ...
        if (self.direction == SLSlideMenuDirectionLeft) {
            ...
            [self.menuView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                CGRect subFrame = obj.frame;
                subFrame.origin.x = [[weakself.startX_arr objectAtIndex:idx] floatValue];
                obj.frame = subFrame;
            }];
        }
        
        if (self.direction == SLSlideMenuDirectionTop) {
            self.menuView.k_y = [[weakself.startY_arr objectAtIndex:0] floatValue];
            
            [self.menuView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//                CGRect subFrame = obj.frame;
//                subFrame.origin.y = [[weakself.startY_arr objectAtIndex:0] floatValue];
//                obj.frame = subFrame;
                
//                self.menuView.k_y = -kAppOriginY;
//                obj.hidden = YES;
            }];
        }
        
//        self.shadowView.backgroundColor = [shadowColor colorWithAlphaComponent:0.0f];
    } completion:^(BOOL finished) {
...
- (void)addGestureRecognizerForMenuView:(UIView *)menuView direction:(SLSlideMenuDirection)direction {
    ...
    recognizer = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(viewTap:)];
