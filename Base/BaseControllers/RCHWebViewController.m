...
@interface RCHWebViewController ()
...
@implementation RCHWebViewController
...
- (void)viewDidLoad {
    ...
    if ([self.parentViewController isKindOfClass:[UINavigationController class]]) {
        ...
        self.webView.frame = (CGRect){{frame.origin.x, self.k_navgationBar.k_height}, {frame.size.width, frame.size.height - self.k_navgationBar.k_height}};
    }
    
    RCHWeak(self);
    [self.webView addObserverBlockForKeyPath:RCHKeyPath(weakself.webView, estimatedProgress) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    [self.webView addObserverBlockForKeyPath:RCHKeyPath(self.webView, title) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
        
        if (!RCHIsEmpty(newVal) && [newVal isKindOfClass:[NSString class]] && [self webViewController:self webViewIsNeedAutoTitle:self.webView]) {
    ...
    [self.webView.scrollView addObserverBlockForKeyPath:RCHKeyPath(self.webView.scrollView, contentSize) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    }else if (!RCHIsEmpty(self.contentHTML)) {
...
- (UIImage *)RCHNavigationBarRightButtonImage:(UIButton *)rightButton navigationBar:(RCHNavigationBar *)navigationBar
{
    ...
    UIImage *image = RCHIMAGEWITHNAMED(@"refresh");
    return image;
}
...
- (UIView *)RCHNavigationBarLeftView:(RCHNavigationBar *)navigationBar
{
    ...
    self.backBtn.k_origin = CGPointMake(0.0f, 0.0f);
    self.closeBtn.k_x = self.backBtn.k_width - 2.0f;
...
#pragma mark -
#pragma mark -  WDNavUIBaseViewControllerDelegate
...
#pragma mark - RCHWebViewControllerDelegate
// 导航条左边的返回按钮的点击
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(null_unspecified WKNavigation *)navigation {
    ...
    if (self.bridge) {
        [self.bridge didFinishLoad:webView];
    }
}
...
- (WKWebView *)webView
{
    if(_webView == nil)
    {
        ...
        // 检测各种特殊的字符串：比如电话、网站
//        if ([WKWebViewConfiguration instancesRespondToSelector:@selector(dataDetectorTypes)]) {
...
//        [config.userContentController addScriptMessageHandler:weakself name:<#(nonnull NSString *)#>];
        ...
        self.bridge = [RCHWebViewJSBridge bridgeWebView:_webView];
    }
    return _webView;
}
...
- (UIProgressView *)progressView
{
    if(_progressView == nil && [self.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        progressView.k_height = 1;
        
        progressView.k_width = kScreenWidth;
        
        progressView.k_y = self.k_navgationBar.k_height;
        progressView.tintColor = kWebProgressColor;
